...
def fetch_stock_data(symbol: str, period: str = "6mo"):
    ...
    # Get historical data
    history = fetcher.get_historical(symbol, period=period, interval="1d")
...
def render_limit_orders(ta_result: TechnicalAnalysisResult):
    """Render limit order recommendations - UX Section 3"""
    ...
    with col1:
        ...
        if ta_result.buy_recommendations:
            for i, rec in enumerate(ta_result.buy_recommendations[:3]):
                with st.container():
                    st.markdown(f"""
                    <div style="background: #1a2e1a; border-radius: 8px; padding: 15px; margin: 5px 0; border-left: 3px solid #00C853;">
                        ...
                        <p style="margin: 5px 0; color: #888;">
                            ...
                            ({rec.expected_return:+.1f}%)
                        </p>
                        <p style="margin: 5px 0; color: #888;">
                            Stop Loss: ${rec.stop_loss:.2f} | R:R {rec.risk_reward_ratio:.1f}x
                        </p>
                        <p style="margin: 5px 0; color: #666; font-size: 12px;">
                            Probability: {rec.probability:.0f}% | {rec.timeframe}
                        </p>
            ...
            st.info("No BUY recommendations at current levels")
    
    # SELL recommendations
    with col2:
        st.markdown("### 🔴 SELL Targets")
        
        if ta_result.sell_recommendations:
            for i, rec in enumerate(ta_result.sell_recommendations[:3]):
                with st.container():
                    st.markdown(f"""
            ...
            st.info("No SELL recommendations at current levels")
...
def render_price_chart(symbol: str, history, ta_result: TechnicalAnalysisResult):
    ...
    if ind.sma_50 > 0 and len(history) >= 50:
    ...
    # Add Bollinger Bands
    if ind.bollinger_upper > 0:
        bb_upper = history['Close'].rolling(20).mean() + 2 * history['Close'].rolling(20).std()
        bb_lower = history['Close'].rolling(20).mean() - 2 * history['Close'].rolling(20).std()
...
def main():
    ...
    with st.spinner(f"Loading data for {symbol}..."):
        try:
            quote, history, info = fetch_stock_data(symbol, period)
        except Exception as e:
            st.error(f"Error fetching data: {e}")
            return
    
    # Convert history to DataFrame if needed
    if history is not None and hasattr(history, 'df'):
    ...
    if history_df is not None and len(history_df) > 0:
        with st.spinner("Running technical analysis..."):
            ta_result = run_technical_analysis(symbol, history_df)
    else:
    ...
    # Row 5: Charts
    render_price_chart(symbol, history_df, ta_result)
    render_rsi_chart(history_df)
    
    st.divider()
    
    # Row 6: Prediction Accuracy
